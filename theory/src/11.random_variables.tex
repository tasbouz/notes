...
experiment is tossing a coin, we can map the outcomes to a r.v $X$ that takes two possible values: 0 for \say{head}
and 1 for \say{tail}. That way we mapped the outcomes of the random experiment to something that we can work with! \v
...
quantity is called \say{probability mass function}.
...
random experiment ($P_{X}(X=x)$). The term \say{mass} helps to get the intuition since the physical mass is conserved
...
variance in absolute terms. For that reason we define a more handy measure of dispersion called \say{standard
deviation}.
...
Another way of describing the discrete uniform distribution would be \say{a known, finite number of outcomes equally
likely to happen}.
...
possible outcomes labelled by $x=1$ (called \say{success}) that occurs with probability p ($ 0<p<1$) and $x=0$
(called \say{failure}) that occurs with probability $q=1-p$. \v
...
\be
For example, it can be used to represent a coin toss where 1 and 0 would represent \say{heads} and \say{tails} (or
...
the first time a \say{1} appears. The probability distribution of the number of times it is thrown is supported on
...
r.v's, this quantity is called \say{probability density function}.
...
Once again, as before, the conservation of density in the continuous case gives a kind of \say{physical} meaning to
$f_{X}$. \v
...
\say{bell curve}) is probably the most important distribution in statistics and it is often used in the natural and
...
useful, we can actually find the value for the integral for any $z$, in the so called \say{$Z$-table}.
...
be seen as \say{standard deviations away from the mean}. For example $z=0.69$ means 0.69 standard deviations away
...
The \say{68–95–99.7 rule} is used in order to get some informal intuitions out of a standard normal distribution.
Also, we can use the \say{68–95–99.7 rule} even for a normal distribution since, as we will show next, any normal
distribution can be turned to a standard normal distribution by a process called \say{standardization}. \v
...
standard normal distribution. This process is called formally \say{standardization}.
...
range by switching to $Z$ and compute the Gaussian integral. Also the \say{68–95–99.7 rule} holds for any normal
...
the whole collection of r.v's as an entity) leading to the concept of a \say{joint probability distribution}.
...
Subsequently, we can define the concept of a \say{joint probability mass function} and a \say{joint probability
...
with \say{independent identical distributed random variables} or i.i.d r.v's. This abbreviation is used a lot in
...
Now we can generalize the concept of expected value to the one of \say{joint expected value}. (Since we are dealing
...
r.v's. The corresponding quantity is called \say{covariance}.
...
The result is a very useful measure in statistics called \say{correlation}.
...
\bd[Hypertailedness]
\textbf{Hypertailedness} is a measure of the \say{hypertailedness} of the probability distribution of a r.v about its
mean:
\bse
...
entity which is called \say{moments}. We distinguish between three different kinds of moments: raw, central and
standardized.
