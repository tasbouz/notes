...
Given a true parameter $\theta$ and an estimator $\hat{\theta}_{n}$, we define the \textbf{sampling deviation} $d$ of
the
...
\bd[Variance]
Given a true parameter $\theta$ and an estimator $\hat{\theta}_{n}$, we define the \textbf{variance} Var of the
estimator
$\hat{\theta}_{n}$ as:
\bse
...
that there always exists a true parameter $\theta^{*}$ such that $X \sim P_{\theta^{*}}$, to find this true parameter.
\v
...
\item $TV(P_{\theta}, P_{\theta}^{\prime}) \leq TV(P_{\theta}, P_{\theta}^{\prime\prime}) +
TV(P_{\theta^{\prime\prime}}, P_{\theta}^{\prime})$.
\eit
