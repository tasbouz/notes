...
Given these values we can calculate the probability of obtaining a value between standard deviations:
\bit
...
For $X$ it is:
\bse
P_{X}(X=0) = \sum_{y} P_{X,Y}(X=0, Y=y) = P_{X,Y}(X=0, Y=0) + P_{X,Y}(X=0, Y=1) = \frac{1}{3} + \frac{1}{6} =
\frac{1}{2}
\ese
...
For $Y$ it is:
\bse
P_{Y}(Y=0) = \sum_{x} P_{X,Y}(X=x, Y=0) = P_{X,Y}(X=0, Y=0) + P_{X,Y}(X=1, Y=0) = \frac{1}{3} + \frac{1}{3} =
\frac{2}{3}
\ese

and:
\bse
P_{Y}(Y=1) = \sum_{x} P_{X,Y}(X=x, Y=1) = P_{X,Y}(X=0, Y=1) + P_{X,Y}(X=1, Y=1) = \frac{1}{6} + \frac{1}{6} =
\frac{1}{3}
\ese
