...
Aliases can be created by making use of the command \code{alias}. After the command, we give our alias a name (by
convention in capital letters) followed immediately (no whitespace allowed) by an equal sign, followed immediately (
...
\item \textbf{~/.zprofile}: A user's personal configuration script for login shells that applies only to the user. It
...
\item \textbf{~/.zshrc}: A user's personal configuration script for interactive non-login shells that applies only to
the user. It can be used to extend or override settings in the global configuration script in \code{/etc/zshrc}.
\eit

The \code{~/.zshrc} startup file is probably the most important startup file from the ordinary user's point of view,
because it is almost always read. Non-login shells read it by default, and most startup files for login shells are
written in such a way as to read the \code{~/.zshrc} file as well. Inside \code{~/.zshrc} we can include anything we
...
Before we finish this section here is a list of useful commands that you might need:
\begin{bash}
# sort results
sort
\end{bash}
...
\bd[Jobspec]
A \textbf{jobspec} \%n is a numeric way of referring to a job.
\ed
...
to place it in the background by appending the trailing \code{&}). \v
