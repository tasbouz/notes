...
\fig{discretevscontinuousprocess}{0.55}
...
\bd[Irreducible Markov Chain]
A Markov chain is called \textbf{irreducible} if it is possible to go anywhere from anywhere.
\ed

\bd[Recurrent State]
A state is called \textbf{recurrent} if the probability of ending in this state is 1.
\ed

\bd[Transient State]
A state is called \textbf{transient} if the probability of ending in this state is less than 1.
\ed
...
\bse
\pi_{2} = \pi_{1} \cdot P = (\pi_{0} \cdot P) \cdot P = \pi_{0} \cdot P^2
\ese
...
\fig{browniannowheredifferentiable}{0.5}
\een

Here is a sample path of a Brownian motion:

\fig{brownianmotion}{0.5}
...
&= E \Big[ \exp \Big\{ \sigma B_{t} + \sigma B_{s} - \sigma B_{s} - \frac{1}{2} \sigma^{2} t \Big\} \: \Big| \: \cF_{s}
\Big] \\
&= E \Big[ \exp \{ \sigma (B_{t} - B_{s}) \} \cdot \exp \Big\{ \sigma B_{s} - \frac{1}{2} \sigma^{2} t \Big\} \: \Big|
\: \cF_{s} \Big] \\
...
& \text{(with %! suppress = EncloseWithLeftRight
$X_{i} \sim N \big(0, \frac{T}{n}\big)$)} \\
...
& \lim_{n \to \infty} \sum_{i=0}^{n-1} (B_{t_{i+1}} - B_{t_{i}})^{2} = \lim_{n \to \infty} \sum_{i=0}^{n-1} (t_{i+1} -
t_{i}) \imp \\
& \sum_{i=0}^{n-1} \lim_{n \to \infty} (B_{t_{i+1}} - B_{t_{i}})^{2} = \sum_{i=0}^{n-1} \lim_{n \to \infty} (t_{i+1} -
t_{i}) \imp \\
