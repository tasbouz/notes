...
\bi{rCl"s}
\nabla_X Y & = & \nabla_{\left(X^i \cibasis{x^i}\right)} \left(Y^m \cibasis{x^m}\right) & (expressed in chart (U,x))
\\[5pt]
...
Now we move on to consider the action of $\nabla_X$ in the case of a $(0,1)$ tensor field, i.e.\ a covector field
$\omega$:
\bi{rCl"s}
\nabla_X \omega & = & \nabla_{\left(X^i \cibasis{x^i}\right)} \left(\omega_m dx^m\right) & (expressed in chart
(U,x))\\[5pt]
...
& = & \cibasis[y^i]{x^q} dx^q \left(\nabla_{\cibasis[x^p]{y^j} \cibasis{x^p}} \cibasis[x^s]{y^k} \cibasis{x^s}
\right)\\[5pt]
...
&= \Gamma^{c}_{ab} \delta^{i}_{c} -\Gamma^{c}_{ba} \delta^{i}_{c} = \Gamma^{i}_{ab} - \Gamma^{i}_{ba} = 2
\Gamma^{i}_{[ab]}
\end{align*}
