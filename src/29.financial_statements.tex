...
The balance sheet presents what the enterprise has today (assets), how much the enterprise owes today (liabilities), and
what the enterprise is worth today (equity). (We will define all of them properly in the next of this section). \v
...
Recall that in a previous chapter we defined the concept of the economic value\eqref{eqn:economic_value} as:
\bd[Economic Value]
...
By using this definition we can define the concept of an asset as:
...
\be
In simple words assets are everything you've got. For example cash in the bank, inventory, machines, buildings, etc.
Assets are also certain rights you own that have a monetary value, like the right to collect cash from customers who
owe you money.
\ee

Assets are valuable, since they represent value of ownership that can be converted into cash (although cash itself is
also considered an asset). The balance sheet of a firm records the value of the assets owned by a business, hence it
is important that this value is quantifiable, for an asset to be listed on the balance sheet. \v

Assets can be grouped into two major classes: tangible assets and intangible assets.

\bd[Tangible Asset]
A \textbf{tangible asset} is literally anything that can be touched.
\ed

\be
Examples of tangible asset are real property like buildings or machinery like an iphone.
\ee
...
\be
Examples of intangible asset are goodwill, copyrights, trademarks, patents, computer programs, financial investments,
bonds, and stocks.
\ee

An intangible asset is usually very difficult to valuate. They suffer from typical market failures of non-rivalry and
non-excludability. \v

Assets are displayed in the asset section of the balance sheet in the descending order of liquidity.

\bd[Liquidity]
\textbf{Liquidity} refers to the ease with which an asset, or security, can be converted into ready cash without
affecting its market price.
\ed

Based on liquidity, assets are divided into three subclasses: current assets, other assets, and fixed assets.
...
purchased price, for this reason some times they are noted as ``fixed assets at cost''.
...
\bd[Liability]
A \textbf{liability} is the future sacrifices of economic benefits that an entity is
obliged to make to other entities as a result of past transactions or other past events, the settlement of which
may result in the transfer or use of assets, provision of services or other yielding of economic benefits in the
future.
\ed

\be
Liabilities are economic obligations of the enterprise, such as money that the corporation owes to lenders, suppliers,
and employees.
\ee
...
As usual, income can have many different parts. Let's them.
...
An \textbf{cash flow statement} is a financial statement that shows how changes in balance sheet and income statement
