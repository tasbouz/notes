...
\bd[Kubeflow]
\textbf{Kubeflow} is an open-source platform for machine learning and MLOps on Kubernetes introduced by Google.
\ed

Kubeflow solves the problem of how to take machine learning from research to production by providing tools to help
with model productionization. In essence, Kubeflow is a collection of cloud native tools for all of the stages of
model development life cycle:
\bit
\item \textbf{Data Exploration}: Model development life cycle always begins with data exploration to understand where
possible insight might exist. In Kubeflow, you can spin up instances of Jupyter that directly interact with your
cluster and its other components.
\item \textbf{Feature Preparation}: Machine learning algorithms require good data to be effective, and often special
tools are needed to effectively extract, transform, and load data. Kubeflow supports a few different tools for this,
like Apache Spark and TensorFlow Transform. These distinct data preparation components can handle a variety of
formats and data sizes and are designed to play nicely with your data exploration environment.
\item \textbf{Model Training}:Once your features are prepped, you are ready to build and train your model. Kubeflow
supports a variety of distributed training frameworks like TensorFlow and PyTorch.
\item \textbf{Hyperparameter Tuning}: With Kubeflow, users can begin with a training model that they are unsure about,
define the hyperparameter search space, and Kubeflow will take care of the rest—spin up training jobs using different
hyperparameters, collect the metrics, and save the results to a model database so their performance can be compared.
\item \textbf{Model Validation}: The same tool used for hyperparameter tuning can perform cross-validation for model
validation. When you’re updating existing models, techniques like A/B testing and multi-armed bandit can be used in
model inference to validate your model online.
\item \textbf{Inference}: Kubeflow makes it easy to deploy machine learning models in production environments at
scale, by providing a multi-framework component for model serving, in addition to existing solutions like TensorFlow
Serving and Seldon Core. In most situations, by pointing Kubeflow to where your model is stored, a server will be
ready to service requests.
\item \textbf{Monitoring \& Continual Learning}: Once the model is served, it needs to be monitored for performance
and possibly updated. This monitoring and updating is possible via the cloud native design of Kubeflow.
\eit
...
integrated end-to-end pipelines that connect all components of their model development life cycle. Pipelines enable
reusability and governance of experiments. To do this, Kubeflow treats model development life cycle as a machine
learning pipeline and implements it as a graph, where each node is a stage in a workflow. \v
...
Kubernetes for containerization seeking to tackle the problem of simplifying machine learning through three features:
\bit
\item \textbf{Composability}: The core components of Kubeflow come from data science tools that are already familiar
to machine learning practitioners. They can be used independently to facilitate specific stages of machine learning,
or composed together to form end-to-end pipelines. Composability gives you the freedom to mix and match the best tools
for the job.
\item \textbf{Portability}: By having a container-based design and taking advantage of Kubernetes and its cloud
native architecture, Kubeflow does not require you to anchor to any particular developer environment. You can
experiment and prototype on your laptop, and deploy to production effortlessly. Portability is important to avoid
vendor lock-in.
\item \textbf{Scalability}: By using Kubernetes, Kubeflow can dynamically scale according to the demand on your
cluster, by changing the number and size of the underlying containers and machines. Scalability is important as your
dataset grows.
\eit

\subsection{Components}

In this subsection will explore each component of Kubeflow in detail, and we will explain the role they play in the
overall architecture.

\bit \item \textbf{Central Dashboard}: Your main interface to Kubeflow which allows you to access the majority of
Kubeflow components.
\item \textbf{JupyterHub}: A multiuser hub that spawns, manages, and proxies multiple instances of a single-user
Jupyter notebook. Kubeflow adds kubectl to the provided notebook images, which allows developers to use notebooks to
create and manage Kubernetes resources. This allows for commands execution without leaving the notebook environment.
\item \textbf{Training Operators}: Training components to automate the execution of machine learning algorithms. In
Kubeflow, distributed training jobs are managed by application-specific controllers, known as operators. These
operators extend the Kubernetes APIs to create, manage, and manipulate the state of resources. These operators allow
the automation of important deployment concepts such as scalability, observability, and failover. They can also be
used by pipelines to chain their execution with the execution of other components of the system.
\item \textbf{Pipelines\footnote{A pipeline is, in its essence, a graph of container execution. In addition to
specifying which containers should run in which order, it also allows the user to pass arguments to the entire
pipeline and between participating containers.}}: Allows orchestration of the execution of machine learning
applications by giving the ability to make machine learning jobs repeatable and handle new data. Pipelines are stored
as YAML files. At a high level, the execution of a pipeline\footnote{When running a pipeline you must choose an
experiment. Experiment here is just a convenience grouping for pipeline executions (runs).} contains the following
components:
\bit
\item \textbf{UI}: For managing and tracking pipelines and their execution.
\item \textbf{Python SDK}: An intuitive Domain Specific Language (DSL) in Python to create components or specify a
pipeline in a Pythonic representation. It also allows for some simple Python functions to be used as pipeline stages
without having to explicitly build a container. For each component, Python SDK:
\bit
\item Creates the container, either as a simple Python function, or with any Docker container.
\item Creates an operation that references that container as well as the command line arguments, data mounts, and
variable to pass the container.
\item Sequences the operations, defining which may happen in parallel and which must complete before moving on to a
further step.
\eit
\item \textbf{DSL Compiler}: Compiles pipelines, defined in Python, into a YAML file that Kubeflow Pipelines can
consume.
\item \textbf{Pipeline Service}: Creates a pipeline run from the static configuration.
\item \textbf{Kubernetes Resources}: Calls the Kubernetes API server to create the necessary Kubernetes custom resource
definitions (CRDs) to run the pipeline.
\item \textbf{Orchestration Controllers}: Execute the containers needed to complete the pipeline execution specified by
the Kubernetes resources (CRDs). The containers execute within Kubernetes Pods on virtual machines.
\item \textbf{Artifact Storage}: The Kubernetes Pods store two kinds of data:
\bit
\item \textbf{Metadata}: Experiments, jobs, runs, single scalar metrics, etc. Kubeflow Pipelines stores the metadata in
a MySQL database.
\item \textbf{Artifacts}: Pipeline packages, views, large-scale metrics like time series, etc. Kubeflow Pipelines stores
the artifacts in an artifact store.
\eit
\eit
\item \textbf{Katib}: Allows to perform hyperparameter optimizations by leveraging advanced searching algorithms such
as Bayesian optimization to find optimal hyperparameter configurations. Katib is based on four main concepts:
\bit
\item \textbf{Experiment}: A single optimization run over a feasible space. Each experiment contains a configuration
describing the feasible space, as well as a set of trials. It is assumed that objective function $f(x)$ does not
change in the course of the experiment.
\item \textbf{Trial}: A list of parameter values, $x$, that will lead to a single evaluation of $f(x)$. A trial can
be ``completed'', which means that it has been evaluated and the objective value $f(x)$ has been assigned to it,
otherwise it is ``pending''. One trial corresponds to one job.
...
\eit
\item \textbf{KFServing}: An umbrella implementation which generalizes the model inference concerns of autoscaling,
networking, health checking, and server configuration.
\item \textbf{ML Metadata}: The infrastructure and a library for recording and retrieving metadata associated with an
machine learning workflow. The information, which can be registered in the ML Metadata component includes:
\bit
\item Data sources used for the model’s creation.
\item The artifacts generated through the components/steps of the pipeline.
\item The executions of these components/steps.
\item The pipeline and associated lineage information.
\eit
\eit
