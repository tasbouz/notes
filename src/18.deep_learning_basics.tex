...
We call the first layer of the network (the one with the inputs) the ``input layer'' or the ``$0^{\textit{th}}$
layer''. Each subsequent layer with neurons is called ``$1^{\textit{st}}$ hidden layer'', ``$2^{\textit{nd}}$ hidden
...
\boldsymbol{b}^{[1]})}_{\textit{output of first layer}} + \boldsymbol{b}^{[2]})}_{\textit{output of second layer}} +
\boldsymbol{b}^{[3]})}_{\textit{output of third layer}} \:\:\: \ldots}_{\vdots} \:\:\:) +
\boldsymbol{b}^{[L]})}_{\textit{output of $l-1$ layer}}
\ese
...
\underbrace{
\begin{bmatrix}
\boldsymbol{b}^{[l]} & \boldsymbol{b}^{[l]} & \ldots & \boldsymbol{b}^{[l]} \end{bmatrix}}_{\textit{$m$ times}}
=\begin{bmatrix}
...
At this point we can consider the Lagrangian $\mathcal{L}$ as a ``new'' loss function $J_{\textit{new}}$ that we need
