...
based on examples of \say{input - output} pairs called a \say{training set}.
\ed
...
where $w_0$ is called \say{bias} or \say{intercept} and the rest $w_i$'s are called \say{weights}. We usually refer
to weights and bias as the \say{parameters} of the regression (or the model) and they are the ones that we try to
...
Notice that linear regression does not \say{allow} any polynomial terms of second or higher degree, thus the naming.
...
regression. This technique is called \say{polynomial regression}. \v
...
biggest parts of machine learning and we can do so with the so called \say{optimization techniques}.
...
Probably the most straight forward optimization technique is the so called \say{normal equation}. Since we are
...
This final expression is called \say{Normal Equation}, and it is an exact analytical solution that gives the
parameter vector. \v
...
on the the most standard optimization technique used today called \say{gradient descent}.
...
The hyperparameter $\alpha$ is called \say{learning rate} and defines how big or small steps we take after each
...
learning rate in a \say{right} value is a topic by itself and it is quite heavily researched today.
...
the whole training set as the \say{batch}. For that reason, the usual terminology for what we have seen so far is
...
set for every iteration becomes computationally expensive. For really large dataset with million of points in the
training data, in order to make just one update to the parameters the algorithm needs to make million of calculations. 
...
of $\frac{m}{b}$ training examples each, called \say{mini-batches}, and we update the parameters using each of the
...
subject matter expertise to be able to come up with useful features. In general, because of the importance and the
ubiquity of feature engineering in machine learning projects, there have been many techniques developed to streamline
the process.

\subsection{Removing Features}

In general it's a good practice to explore and visualize the data in order to identify useless information. In this
step you can drop the attributes that provide no useful information for the task, drop variables that have a very low
variation (i.e.\ not too much information), drop variables that have very low correlation with the target, and
finally find variables that are highly correlated with each other (i.e.\ same behaviour), and keep the ones that have
...
This is one of the simplest things you can do that often results in a performance boost for your model. Neglecting to
do so can cause your model to make gibberish predictions. There are two common ways to get all attributes to have the
same scale: ``normalization'' and ``standardization''.
...
don't accidentally gain information about the test split.

\subsection{Discretizating Features}
...
features can be bad both during training and serving your model for many reasons. First of all, the more features you
have, the more opportunities there are for data leakage. Secondly, too many features can cause overfitting. From a
technical perspective, too many features can increase memory required to serve a model, which, in turn, might require
you to use a more expensive machine/instance to serve your model. Also, too many features can increase inference
latency when doing predictions. Last but not least, useless features become technical debts. Whenever your data
pipeline changes, all the affected features need to be adjusted accordingly. \v
...
(The argument of the logistic function is often called \say{logit}). \v
...
loss function for linear regression. The name \say{cross entropy} comes from the definition of cross entropy which is
...
\say{softmax regression}. However, since softmax regression is used heavily in deep learning we will develop its
...
follows a Bernoulli distribution. We can generalize both regressions in one coherent model called \say{generalized
...
variance. This is why it is called a \say{trade-off}. \v

It is worth mentioning that we have neglected the so-called \say{irreducible error} for the equations. This part is
...
an area of heavy research, but in general in machine learning we usually split them in a proportion of \say{80\% -
20\%}. In other areas of machine learning like deep learning where we usually have a very large amount of data we use
splitting rules of \say{99\% - 1\%}. But we will address this issue in details in deep learning chapter. \v
...
improving the model. For the later, what people usually do is to further split the train set to a \say{new} (even
smaller) train set, and a so called \say{evaluation set} (using again a proportion of \say{80\% - 20\%}) and then the
...
There are many different regularization techniques. We will begin with four main ones called \say{Ridge regression}
(or \say{Tikhonov regression} or \say{L2 regularization}), \say{Lasso regression } (or \say{L1 regularization}),
\say{elastic net}, and \say{early stopping}.
...
where $C$ is a constant defined by us and it controls the effect of regularization. It is called \say{L2
...
regression is called \say{Ridge regression} (or \say{Tikhonov regularization}), where the only difference with linear
...
The corresponding regression is called \say{Least Absolute Shrinkage and Selection Operator Regression}, usually
simply called \say{Lasso regression}. An important characteristic of Lasso regression is that it tends to eliminate
...
as the validation error reaches a minimum. This is called \say{early stopping}. In general as the algorithm learns,
...
Then we gather them all together to the so called \say{confusion matrix}.
...
vice versa. This is called the \say{precision/recall trade-off}. \v
...
the harmonic mean gives much more weight to low values. and as a result, a classifier will only get a high $F_1$
score if both recall and precision are high.

\subsection{Receiver Operating Characteristic (ROC) Curve}

Another very widely used technique in classification evaluation is the so called \say{receiver operating
...
We would then predict 1 on an input $\boldsymbol{x}$ if and only if $h (\boldsymbol{w}^{\intercal} \boldsymbol{x})
...
(\boldsymbol{w}^{\intercal} \boldsymbol{x})$ a.k.a the larger $P(y = 1| \boldsymbol{x}; \boldsymbol{w})$ is, and thus
...
$\boldsymbol{w}$ with $2\boldsymbol{w}$ and $b$ with $2b$, then since $g (\boldsymbol{w}^{\intercal} \boldsymbol{x} +
b) = g(2\boldsymbol{w}^{\intercal} \boldsymbol{x} + 2b)$ this would not change $h_{\boldsymbol{w}, b}
...
optimal solution to our optimization problem. These three points are called \say{support vectors}. The fact that the
...
the KKT multipliers. So we ended up with the so called \say{dual optimization problem} which is:
\bse
...
This model of classification is called \say{hard margin support vector machine}. A careful reader would notice that
...
In this case the final model is called \say{soft margin support vector machine} (in contrast with the SVM we
developed in the beginning called \say{hard margin support vector machine}), due to the fact that it allows the
...
In this section we will introduce yet another famous supervised learning algorithm called \say{k-nearest neighbors
...
One good question is \say{how to measure the distance between two points}? There are many ways of calculating
...
Another good questions is \say{how many neighbors should I choose for the model}? In other words what should be the
...
called the \say{nearest neighbor} algorithm. \v

\fig{knn}{0.55}

$k$-NN has many advantages. First of all the algorithm is very simple and easy to implement. Also, there is no need
to build a model, tune several parameters, or make additional assumptions. and finally the algorithm is versatile, i.e.\ 
...
\say{top-down} approach. Each node in the tree acts as a test case for some attribute, and each edge descending from
that node corresponds to one of the possible answers to the test case. \v

The algorithm begins by selecting the best attribute using something called \say{attribute selection measures} to
...
possible manner. It is also known as \say{splitting rules} because it helps us to determine breakpoints for tuples on
...
Two of the most popular selection measures are: \say{information gain}, and \say{Gini impurity} and we will now
...
the training examples according to their target classification. It is a synonym for \say{Kullback-Leibler
...
will find that this aggregated answer is better than an expert's answer. This is called the \say{wisdom of the crowd}. 
