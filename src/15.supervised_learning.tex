...
Evaluation is about how good a model generalizes to new data. After applying the learning algorithm to the data and
having obtained a hypothesis $h$, the machine learning model is ready to make new predictions. However before that,
we have to evaluate the model by analysing the errors that we just introduced. \v
...
Support vector machine (SVM) is another, more advanced supervised learning algorithm. It applies mainly in
classification problems however there is also another model called support vector regression that applies the same
ideas in regression problems. Here we will explore only SVM. \v
...
\qquad \textit{and} \qquad || \boldsymbol{w} || = 1
\ese
...
where $m$ is the total number of instances in both left and right subsets, $m_{\textit{left}}$ and
$m_{\textit{right}}$ are the number of instances in the left and right subset, and $G_{\textit{left}}$ and
$G_{\textit{right}}$ measure the impurity of the left and the right subset ($G$ could be either information gain or
Gini impurity).
\ed
...
\bse
J(k, t_{k}) = \frac{m_{\textit{left}}}{m} \textit{MSE}_{\textit{left}}
+ \frac{m_{\textit{right}}}{m}\textit{MSE}_{\textit{right}}
\ese

where $m$ is the total number of instances in both left and right subsets, $m_{\textit{left}}$ and
$m_{\textit{right}}$ are the number of instances in the left and right subset, and $\textit{MSE}_{\textit{left}}$ and
$\textit{MSE}_{\textit{right}}$ are the mean squared errors of the left and the right subsets given by:
\bse
