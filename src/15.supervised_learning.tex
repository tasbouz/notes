...
\bd[Weighted Gini Impurity \ Weighted Gini Index]
Given a discrete random variable $X$, with possible outcomes $\{x_{1}, x_{2}, \ldots, x_{n} \}$ which occur with
...
weights, given by:
\bse
e = \frac{\sum_{i=1}^{m} w^{(i)} \cdot \mathbb{I}(\hat{y}^{(i)} \neq y^{(i)})}{\sum_{i=1}^{m} w^{(i)}}
\ese
...
w^{(i)}_{\textit{new}} =
\begin{cases}
w^{(i)} \cdot \exp(\alpha) & \text{if } \hat{y}^{(i)} \neq y^{(i)} \\
w^{(i)} \cdot \exp(-\alpha) & \text{if } \hat{y}^{(i)} = y^{(i)}
\end{cases}
\ese
...
\bse
\hat{y} = \frac{1}{1 + \exp^{-\log{\frac{p}{1-p}}}}
\ese
...
In classification one needs to convert this final prediction to a probability by using the logistic function:
\bse
\hat{y} = \frac{1}{1 + \exp^{-\textit{(sum of all the learning-rate weighted pseudo-residuals)}}}
\ese
...
\bd[Extreme Gradient Boosting (XGBoost)]
...
\bd[Quality Score / Similarity Score]
...
\bse
s = \frac{\sum_{i=1}^{m} e^{(i)^2}}{m + \lambda}
\ese
\ed
...
\bse
g = s_{\text{parent}} - (s_{\text{left}} + s_{\text{right}})
\ese
\ed
...
\bse
\hat{y} = \frac{\sum_{i=1}^{m} e^{(i)}}{m + \lambda}
\ese
