...
(e_1,\ldots,e_{\dim M}) \Leftrightarrow (g^a_{\phantom{a}1}e_a,\ldots, g^a_{\phantom{a}\dim M}e_a)= (e_1,\ldots,e_{\dim
M})
\ese
...
Finally, we have:
\bse
(\rho\circ\id_{LM})(e_1,\ldots,e_{\dim M}) = \GL(\dim M,\R)_{(e_1,\ldots, e_{\dim M})} = (f\circ \pi)(e_1,\ldots,
e_{\dim M})
\ese
...
\bi{rrCl}
\Phi^*(\omega)\cl & \Gamma(TM) & \xrightarrow{\sim} & \mathcal{C}^\infty(M)\\ & X & \mapsto & \omega(\Phi_*(X))\circ
\phi
\ei
...
\adjustbox{scale=1.3,center}{
\begin{tikzcd}
&P \ar[ddl,"\pi"'] \ar[ddr,"\pi"] &\\ &&\\ U_1 \ar[uur,bend left=80,"\sigma_1"]&& U_2 \ar[uul,bend right=80,
"\sigma_2"']
\end{tikzcd}}
\ese
...
\item $\forall \, \lambda\in [0,1] : \ \pi_*(X_{\gamma^{\uparrow}, \gamma^{\uparrow}(\lambda)})=X_{\gamma,\gamma
(\lambda)}$.
\een
\ed
...
\bi{rrCl}
\D\phi\cl & \Gamma(TP)^{\times (k+1)} & \to & V\\ & (X_1,\ldots,X_{k+1}) & \mapsto & \d \phi
(\hor(X_1),\ldots,\hor(X_{k+1})).
\ei
\ed
...
and:
\bse
(\omega\Wedge\omega )(X,Y) = \left\llbracket \omega(X),\omega(Y) \right\rrbracket = \left\llbracket 0, 0\right\rrbracket
= 0
\ese
