...
denotes the group as $\GL(n,V)$ where $n$ indicates the number of basis vectors (which is of course the dimension of
...
Recall that if $B$ is a bilinear form on a $K$-vector space $V$, the determinant of $B$ is not independent from the
...
The definition formalises the idea that the assignment of an $H_p P$ to each $p\in P$ should be ``smooth'' within
...
one-form on the vector space $T_m U$. By using the co-ordinate induced basis and its dual basis, we can express $
...
the identity $\id_{\R^d}$, and let $x^i_{\phantom{i}j}\cl \GL^+(d,\R)\to \R$ denote the corresponding co-ordinate
functions:
...
so that $x^i_{\phantom{i}j}(g) \coloneqq g^i_{\phantom{i}j}$. Recall that the co-ordinate functions are smooth maps
...
Note that since the $G$-action $\racts$ on $P$ is free, for each $m$ there exists a unique $\Omega(m)$ satisfying the
...
As an application, consider the spacial case in which the sections $\sigma_1,\sigma_2$ are induced by co-ordinate
...
\bse
\gamma^{\negmedspace\stackrel{P_F}{\uparrow}}_{[\sigma(m),f]}(t) \qquad \textit{and} \qquad \sigma(\gamma(t))
\ese
...
anti-symmetric, $\mathcal{C}^\infty(P)$-multilinear map) with values in some module $V$. Then then \textbf{exterior
