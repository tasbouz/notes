...
\begin{bash}
# edit the latest commit’s message nothing must be staged
...
\fig{img/Kubernetes_logo_without_workmark}{0.04}
...
partnership with the Linux Foundation. The design and development of Kubernetes was influenced by Google's Borg
cluster manager. Many of its top contributors had previously worked on Borg; they codenamed Kubernetes ``Project 7''
after the Star Trek ex$-$Borg character Seven of Nine and gave its logo a seven$-$spoked wheel. Unlike Borg, which
was written in C++, Kubernetes source code is in the Go language. \v
...
Containerd, and CRI$-$O. In February 2016, the Helm package manager for Kubernetes was released.

\subsection{Components}

Kubernetes consists of a large collection of components, each one with each own usage. It is crucial to understand
...
(more specifically with the cluster) in order to create and manipulate all the components we have just introduced. \v
...
Kubectl can be installed via Homebrew:
...
In the previous subsection we gave a minimalistic example of creating a deployment, where we just specified the name
...
to delete a cluster, we will simply delete the configuration file that created the cluster:
...
Finally, let's create the Mongo Express inside the cluster:

\begin{bash}
# create mongo express component
