...
for any $f\in \mathcal{C}^\infty(U)$. Hence, $\lambda \odot X_{\gamma,p}=X_{\sigma,p}\in T_p M$. Hence, indeed $T_p M$
is a vector space. \qedhere
\een
\eq
...
\bi{rCl}
((\phi^*)_p)_a & = & (\phi^*)_p \left(\left(dy^a\right)_{\phi(p)}\right)
\left(\left(\cibasis{x^i}\right)_p\right)\\[5pt]
...
\textbf{pull-back bundle of} $E\xrightarrow{\,\pi\, }M$ \emph{induced by} $f$ is defined as
$E'\xrightarrow{\,\pi'\,}M'$,
where:
\bse
...
In the case of $\R$-vector space we can only assign the same value to every point (i.e.\ having a constant vector
field).
