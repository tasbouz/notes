...
Hence, a two-dimensional image can be represented as a $(\textit{height} \times \textit{width} \times 
\textit{depth})$ dimensional matrix. Subsequently, a collection of $N$ such images can be stacked together to a $(N 
\times \textit{height} \times \textit{width} \times \textit{depth})$ dimensional matrix. This matrix is the 
...
of it e.g: $\hat{y} = \textit{sentiments(words)}$. Say, we employ a machine learning algorithm (some mathematical
...
$W^{[1]}$ simply $W_{\textit{context}}$. Notice that we carry no biases. The pre-activations are:
...
Notice that the product $W_{\textit{context}} \cdot \textbf{x}$, given that $\textbf{x}$ is a one hot-vector, is
simply the $\textit{i}^{\textit{th}}$ column of $W_{\textit{context}}$ since everything else will be multiplied by 0.
So when the $\textit{i}^{\textit{th}}$ word is present the $\textit{i}^{\textit{th}}$ element in the one-hot vector
is on and the $\textit{i}^{\textit{th}}$ column of $W_{\textit{context}}$ gets selected. In other words, there is a
one-to-one correspondence between the words and the column of $W_{\textit{context}}$. More specifically, we can treat
the $\textit{i}^{\textit{th}}$ column of $W_{\textit{context}}$ as the representation of the
$\textit{i}^{\textit{th}}$ context.
...
practice to call $W^{[2]}$ simply $W_{\textit{word}}$. The pre-activations now are:
\bse
...
\bse
\hat{\textbf{y}} = P(\textit{word \:} | \textit{\: context}) = \frac{e^{W_{\textit{word}} \cdot
\textbf{h}^{[1]}}}{\sum_{\textit{words}} e^{W_{\textit{word}} \cdot \textbf{h}^{[1]}}}
\ese
...
and since from the product $W_{\textit{context}} \cdot \textbf{x} \:$ survives only the column of
$W_{\textit{context}}$ associated with the context, it turns out that $P (\textit{word \:} | \textit{\: context})$ is
proportional to the dot product between of $W_{\textit{context}}$ and $W_{\textit{word}}$. In other words the
probability for each word depends on the $\textit{i}^{\textit{th}}$ column of $W_{\textit{word}}$. We thus treat the
$\textit{i}^{\textit{th}}$ column of $W_{\textit{word}}$ as the representation of the $\textit{i}^{\textit{th}}$ word.
\v
...
is:
\bse
\mathcal{L}(\theta) = - \ln P(\textit{word \:} | \textit{\: context}) = - \ln \frac{e^{W_{\textit{word}}
\cdot \textbf{h}^{[1]}}}{\sum_{\textit{words}} e^{W_{\textit{word}} \cdot \textbf{h}^{[1]}}}
\ese
...
\bse
\mathcal{L}(\theta) = - \sum_{\textit{words in window}} \ln P(\textit{word
\:} | \textit{\: context})
\ese
...
\bit
\item \textbf{Exploding Gradients} If $W > 1$ then $W^i \to \infty, \textit{as } i \to n$ which has as a result for
...
\eit
\item \textbf{Vanishing Gradients}: If $W < 1$ then $W^i \to 0, \textit{as } i \to n$ which has as a result the terms
...
The new parameters $U_{\textit{o}}, W_{\textit{o}}$ need to be learned along with the existing parameters $W, U, V$. \v
...
\item By defining the input gate $\boldsymbol{i}_{i} = \sigma(U_{\textit{i}} \boldsymbol{x}_{i} + W_{\textit{i}}
\boldsymbol{h}_{i-1})$ and the forget gate $\boldsymbol{f}_{i} = \sigma (U_{\textit{f}} \boldsymbol{x}_{i} +
W_{\textit{f}} \boldsymbol{h}_{i-1})$ and by using $\boldsymbol{s}_{i-1}$ and $\boldsymbol{\tilde{s}}_i$ compute
\item $\boldsymbol{s}_{i} =\boldsymbol{\tilde{s}}_i\odot \boldsymbol{i}_{i} + \boldsymbol{s}_{i-1} \odot
\boldsymbol{f}_{i}$.
\item By defining the output gate $\boldsymbol{o}_{i} = \sigma (U_{\textit{o}} \boldsymbol{x}_{i} + W_{\textit{o}}
...
(U_{\textit{o}} \boldsymbol{x}_{i} + W_{\textit{o}} \boldsymbol{s}_{i-1})$.
