...
some statistic of $y$ conditioned on $x$. Specialized loss functions allow us to train a predictor of these estimates.
\v
...
& = \frac{\partial J}{\partial \hat{y}} \cdot \frac{\partial h^{[L]}}{\partial a^{[L]}}
\cdot \frac{\partial \left( W^{[L]} \boldsymbol{h}^{[L-1]} + \boldsymbol{b}^{[L]} \right)}{\partial
\boldsymbol{b}^{[L]}} \\
...
all its inputs to the range $[-1,1]$ hence, it is zero centered. However, its gradient still vanishes at saturation
since:
\bse
